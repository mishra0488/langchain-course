...
def get_tool_descriptions(tools_dict):
    ...
    for tool_name, tool_function in tools_dict.items():
        ...
        # means func got its wrapped attribute - that is @traceable
        original_function = getattr(tool_function, "__wrapped__", tool_function)
        # using inspect to get the metadata of function 
        # i.e. function name, arguments it recieves, and their types and return value
        signature = inspect.signature(original_function)
        docstring = inspect.getdoc(tool_function) or "" #getdoc gets the docstring which helps where to use this func
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        print(f"\n--- Iteration {iteration} ---")
        full_prompt = prompt + scratchpad             # now full promp appends original prompt plus scratchpad i.e. history

        # Stop token prevents the LLM from generating its own Observation — or say hallucination
